...
namespace Chinmaya.Registration.DAL
{
    public class Common
    {
		...
		public List<KeyGuidValueModel> GetCenterState(Guid id)
		{
			using (var _ctx = new ChinmayaEntities())
			{
				return _ctx.CM_States.Where(s => s.CountryGUID == id).Select(x => new KeyGuidValueModel
				{
					Id = x.StateGUID,
					Name = x.StateName
				}).ToList();
		...
		public List<KeyGuidValueModel> GetCenters(Guid id)
		{
			using (var _ctx = new ChinmayaEntities())
			{
				return _ctx.CM_Centers.Where(s => s.CityGUID == id).Select(x => new KeyGuidValueModel
				{
					Id = x.CenterGUID,
					Name = x.CenterName
				}).ToList();
