...
namespace Chinmaya.Registration.DAL
{
	...
	public class Users
	{
		...
		public List<TeacherModel> GetTeacherData()
		{
			using (var _ctx = new ChinmayaEntities())
			{
				var teacherData = (from u in _ctx.Users
								   where u.RoleId == 3
								   select new TeacherModel
								   {
									   ...
									   HomePhone = u.HomePhone,
									   CellPhone = u.CellPhone,
									   TeacherDetails = _ctx.TeacherInfoes.Where(r => r.Id == u.Id).Select(n => n.Teacherdetails).FirstOrDefault(),
									   Class = _ctx.Events.Where(e => e.Id == (_ctx.TeacherClasses.Where(r => r.Id == u.Id).Select(n => n.ClassId).FirstOrDefault())).Select(n => n.Name).FirstOrDefault(),
								   }).ToList();
				return teacherData;
			}
		}
		...
		public List<TransactionInfo> GetTransactionInformation(string Id)
		{
			using (var _ctx = new ChinmayaEntities())
			{
				var transactionData = (from c in _ctx.CheckPayments
								   where c.CreatedBy == Id
								   select new TransactionInfo
								   {
									   AccountHolderName = c.AccountHolderName,
									   AccountType = _ctx.AccountTypes.Where(r => r.Id == c.AccountTypeId).Select(r => r.Name).FirstOrDefault(),
									   BankName = c.BankName,
