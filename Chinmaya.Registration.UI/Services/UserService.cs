...
namespace Chinmaya.Registration.UI.Services
{
    public class UserService
    {
		...
		public async Task<List<TeacherModel>> GetTeacherData()
		{
			HttpResponseMessage getTeacherDataResponse = await Utility.GetObject("/api/User/GetTeacherData/", true);
			return await Utility.DeserializeObject<List<TeacherModel>>(getTeacherDataResponse);
		}

		public async Task<UpdateEmail> GetEmailById(string Id)
		{
			HttpResponseMessage roleResponseMessage = await Utility.GetObject("/api/User/GetEmailById/" + Id, true);
			return await Utility.DeserializeObject<UpdateEmail>(roleResponseMessage);
		}

		public async Task<List<TransactionInfo>> GetTransactionInformation(string Id)
		{
			HttpResponseMessage roleResponseMessage = await Utility.GetObject("/api/User/GetTransactionInformation/" + Id, true);
			return await Utility.DeserializeObject<List<TransactionInfo>>(roleResponseMessage);
		}
	}
}