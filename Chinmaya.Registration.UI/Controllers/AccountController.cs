...
namespace Chinmaya.Registration.UI.Controllers
{
	...
	public class AccountController : BaseController
	{
       ...
       [AllowAnonymous]
		...
		[AllowAnonymous]
		public async Task<ActionResult> CenterDetails()
		{
			/*ViewBag.CenterCountryList = await _common.GetCenterCountryData();
			
			if (TempData["CenterDetails"] != null)
			{
				var cnd = TempData["CenterDetails"] as CenterDetails;
				ViewBag.SelectedCenterCountry = cnd.Country;
				ViewBag.SelectedCenterState = cnd.State;
				ViewBag.SelectedCenterCity = cnd.City;
				ViewBag.SelectedCenter = cnd.Center;
				return View(cnd);
			}*/
			return View();
		}
		...
		[HttpPost]
		[AllowAnonymous]
		public async Task<ActionResult> CenterDetails(CenterDetails DetailsData, string BtnPrevious, string BtnNext)
		{
			ViewBag.CenterCountryList = await _common.GetCenterCountryData();
			...
			if (BtnNext != null)
			{
				if (ModelState.IsValid)
				{
					...
					UserObj.CenterCountry = DetailsData.Country;
					UserObj.CenterState = DetailsData.State;
					UserObj.CenterCity = DetailsData.City;
					UserObj.Center = DetailsData.Center;
		...
		public async Task<ActionResult> PersonalDetails(PersonalDetails DetailsData, string BtnPrevious, string BtnNext)
		{
            /*ViewBag.Gender = await _common.GetGenderData();
			ViewBag.AgeGroup = await _common.GetAgeGroupData();
			ViewBag.CountryList = await _common.GetCenterCountryData();
			ViewBag.SelectedCountry = await _account.GetCountryId("United States");
			UserModel obj = GetUser();
			if (BtnPrevious != null)
			{
				CenterDetails cnd = new CenterDetails();
				cnd.Country = obj.CenterCountry;
				cnd.State = obj.CenterState;
				cnd.City = obj.CenterCity;
				cnd.Center = obj.Center;
				TempData["CenterDetails"] = cnd;
				return RedirectToAction("CenterDetails");
			}
			if (BtnNext != null)
			{
				if (ModelState.IsValid)
				{
					...
					if (TempData["RoleId"] != null) UserObj.RoleId = 3;
					else UserObj.RoleId = 2;
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<ActionResult> Confirmationdetail()
        {
            //ViewBag.CountryList = await _common.GetCountryData();
            //ViewBag.CountryList = await _common.GetCenterCountryData();
            //ViewBag.SelectedCountry = await _account.GetCountryId("United States");
            ...
            //	return View(cd);
            //}
            return View("Confirmationdetail");
        }

        [HttpGet]
        [AllowAnonymous]
        public async Task<ActionResult> Confirmationdetail2()
        {
            //ViewBag.CountryList = await _common.GetCountryData();
            //ViewBag.CountryList = await _common.GetCenterCountryData();
            //ViewBag.SelectedCountry = await _account.GetCountryId("United States");
            ...
            //	return View(cd);
            //}
            return View("Confirmationdetail2");
        }
        ...
        [HttpPost]
		[AllowAnonymous]
		public async Task<ActionResult> ContactDetails(ContactDetails data, string prevBtn, string nextBtn, string nextBtn2)
		{
            //UserModel obj = GetUser();
            //ViewBag.CountryList = await _common.GetCenterCountryData();
            //ViewBag.SelectedCountry = await _account.GetCountryId("United States");
		...
		public async Task<ActionResult> PastTransactions()
		{
			List<TransactionInfo> transactionInfo = new List<TransactionInfo>();
			transactionInfo = await _user.GetTransactionInformation(User.UserId);
			return View("PastTransactions", transactionInfo);
		}

		[HttpGet]
		[AllowAnonymous]
		public async Task<JsonResult> FillCenterState(Guid Id)
		{
			HttpResponseMessage roleNameResponseMessage = await Utility.GetObject("/api/Account/GetCenterState/" + Id, true);
			var serializedStates = await Utility.DeserializeList<KeyGuidValueModel>(roleNameResponseMessage);
			return Json(serializedStates, JsonRequestBehavior.AllowGet);
		}

		[HttpGet]
		[AllowAnonymous]
		public async Task<JsonResult> FillCenterCity(Guid Id)
		{
			HttpResponseMessage roleNameResponseMessage = await Utility.GetObject("/api/Account/GetCenterCity/" + Id, true);
			var serializedCities = await Utility.DeserializeList<KeyGuidValueModel>(roleNameResponseMessage);
			return Json(serializedCities, JsonRequestBehavior.AllowGet);
		}

		[HttpGet]
		[AllowAnonymous]
		public async Task<JsonResult> FillCenters(Guid Id)
		{
			HttpResponseMessage roleNameResponseMessage = await Utility.GetObject("/api/Account/GetCenters/" + Id, true);
			var serializedCenters = await Utility.DeserializeList<KeyGuidValueModel>(roleNameResponseMessage);
			return Json(serializedCenters, JsonRequestBehavior.AllowGet);
		}
	}
}